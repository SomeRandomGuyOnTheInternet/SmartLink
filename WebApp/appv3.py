...
def on_message(client, userdata, message):
	...
	if message.topic == CONNECT_REMOTE_RESPONSE:
		...
		publish_message(LEARN_COMMAND_REQUEST,json.dumps(response_payload))
	...
	if message.topic == LEARN_COMMAND_RESPONSE:
		...
		publish_message(SEND_COMMAND_REQUEST ,json.dumps(response_payload))
...
try:
	...
	print("Cannot find remote client!")
...
def on_message_found_command(mqttc, obj, msg):
	payload = json.loads(msg.payload)
	publish_message(SEND_COMMAND_REQUEST, None)
	print("Found command!")
	print(payload)
...
def send_command():
	...
	publish_message(SEND_COMMAND_REQUEST, json.dumps(payload))
	return jsonify({"success": True})
